"""
Multi-LoRA Loader for Qwen
Platform-specific loader for Qwen LoRAs
Filters to show only LoRAs in L:\stable-diffusion-webui\models\Lora\Qwen

This is a model-only loader (no CLIP) optimized for Qwen workflows.
"""
...
class MultiLoRALoaderQwen(MultiLoRALoaderBase):
    """
    Multi-LoRA loader specifically for Qwen LoRAs.
    Only shows LoRAs from the Qwen directory.
    Model-only (no CLIP).
    """
    
    # Platform-specific configuration
    PLATFORM_NAME = "Qwen"
    PLATFORM_DIRECTORY_FILTER = "Qwen"  # Will scan Lora\Qwen
    REQUIRES_CLIP = False  # Qwen doesn't use CLIP
    DEFAULT_ARCHITECTURE = "Qwen"
    MAX_LORA_SLOTS = 8
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "search_category": (["Any", "unknown", "style", "character", "concept", "tool",
                    "enhancement", "effect"], {"default": "Any",
    ...
    def load_multi_loras(self, model, prompt: str,
        ...
        """Load multiple LoRAs for Qwen (model only)"""
        ...
        # Create filter info
        active_filters = []
        active_filters.append(f"Platform: Qwen")
        ...
        filter_info = f"FILTERS: {' | '.join(active_filters)} | Found {len(filtered_lora_paths)} of {len(self.lora_paths)} Qwen LoRAs"
        ...
        for i, (enabled, name, strength) in enumerate(lora_configs, 1):
            ...
            if not lora_path:
                print(f"[Qwen] Warning: LoRA '{name}' not found, skipping slot {i}")
                continue
            ...
            try:
                ...
                loaded_loras.append({
                    ...
                    "name": name,
                    "strength": strength,
                    "architecture": lora_info.get("architecture", "Qwen"),
                ...
                print(f"[Qwen] Loaded LoRA {i}: {name} (strength: {strength})")
                
            except Exception as e:
                print(f"[Qwen] Error loading LoRA '{name}' in slot {i}: {str(e)}")
                continue
...
# Node registration
NODE_CLASS_MAPPINGS = {
    "Multi-LoRA Loader Qwen": MultiLoRALoaderQwen,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "Multi-LoRA Loader Qwen": "Multi-LoRA Loader (Qwen)",
}
