...
class MetadataAwareSaveImageSimple(MetadataAwareSaveImage):
    """Standalone simplified save node without layer, secondary-format, or SVG inputs."""

    @classmethod
    def INPUT_TYPES(cls):
        file_formats = ["png", "jpg", "webp", "tiff", "apng"]
        ...
        return {
            "required": {
                ...
                "filename_prefix": (
                    ...
                    {
                        "default": "ComfyUI",
                        "tooltip": "Base name for saved files. Supports tokens like %date:yyyy-MM-dd%.",
                    },
                ),
                "file_format": (
                    file_formats,
                    {"default": "png", "tooltip": "Primary image format."},
                ),
            },
            "optional": {
                "include_project": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "Append project name to filename."},
                ),
                "include_datetime": (
                    "BOOLEAN",
                    {"default": True, "tooltip": "Append timestamp to filename."},
                ),
                "custom_output_directory": (
                    "STRING",
                    {
                        "default": "",
                        "tooltip": "Override output folder. Relative paths stay under ComfyUI/output.",
                    },
                ),
                "output_path_mode": (
                    ["Absolute Path", "Subfolder in Output"],
                    {
                        "default": "Subfolder in Output",
                        "tooltip": "Treat custom directory as absolute or relative to the output folder.",
                    },
                ),
                "filename_format": (
                    ["Default (padded zeros)", "Simple (file1.png)"],
                    {
                        "default": "Default (padded zeros)",
                        "tooltip": "Choose sequential filename style.",
                    },
                ),
                "quality_preset": (
                    ["Best Quality", "Balanced", "Smallest File"],
                    {
                        ...
                        "tooltip": "Adjust compression vs. quality for supported formats.",
                    },
                ),
                "color_profile": (
                    list(ICC_PROFILES.keys()),
                    {"default": "sRGB v4 Appearance", "tooltip": "Embedded ICC profile."},
                ),
                "rendering_intent": (
                    list(COLOR_INTENTS.keys()),
                    {
                        "default": "Relative Colorimetric",
                        "tooltip": "Rendering intent used for color conversions.",
                    },
                ),
                "bit_depth": (
                    ["8-bit", "16-bit"],
                    {
                        "default": "8-bit",
                        "tooltip": "16-bit currently applies to PNG only.",
                    },
                ),
                "alpha_mode": (
                    ["auto", "premultiplied", "straight", "matte"],
                    {
                        "default": "auto",
                        "tooltip": "How to treat alpha when the format lacks transparency.",
                    },
                ),
                "matte_color": (
                    "STRING",
                    {
                        "default": "#FFFFFF",
                        "tooltip": "Background colour for matte mode (name or #RRGGBB).",
                    },
                ),
                "save_alpha_separately": (
                    ...
                    {
                        "default": False,
                        "tooltip": "Write a separate alpha image for formats without transparency support.",
                    },
                ),
                "apng_fps": (
                    "INT",
                    {
                        ...
                        "min": 1,
                        "max": 60,
                        "tooltip": "Frames per second for APNG output.",
                    },
                ),
                "apng_loops": (
                    "INT",
                    {
                        ...
                        "min": 0,
                        "max": 99999,
                        "tooltip": "Loop count for APNG (0 means infinite).",
                    },
                ),
                "enable_metadata": (
                    "BOOLEAN",
                    {
                        "default": True,
                        "tooltip": "Master switch for writing structured metadata.",
                    },
                ),
                "title": ("STRING", {"default": "", "tooltip": "Image title."}),
                "project": ("STRING", {"default": "", "tooltip": "Project or collection name."}),
                "description": (
                    "STRING",
                    {"default": "", "tooltip": "Image description or caption."},
                ),
                "creator": ("STRING", {"default": "", "tooltip": "Creator/artist name."}),
                "copyright": (
                    "STRING",
                    {"default": "", "tooltip": "Copyright / rights notice."},
                ),
                "keywords": (
                    "STRING",
                    {"default": "", "tooltip": "Comma-separated keyword list."},
                ),
                "custom_metadata": (
                    "STRING",
                    {
                        "default": "{}",
                        "tooltip": "Custom metadata JSON merged into the output structure.",
                    },
                ),
                "auto_extract_generation_params": (
                    "BOOLEAN",
                    {
                        "default": True,
                        "tooltip": "Pull prompts, sampler settings, etc. from the workflow automatically.",
                    },
                ),
                "include_node_details": (
                    ...
                    {
                        "default": False,
                        "tooltip": "Include node IDs and wiring details for debugging.",
                    },
                ),
                "save_embedded": (
                    "BOOLEAN",
                    {
                        "default": True,
                        "tooltip": "Write structured metadata directly into the image when possible.",
                    },
                ),
                "save_workflow_as_json": (
                    ...
                    {
                        "default": False,
                        "tooltip": "Export the workflow alongside the image as JSON.",
                    },
                ),
                "save_xmp": (
                    "BOOLEAN",
                    {
                        "default": True,
                        "tooltip": "Create an XMP sidecar file.",
                    },
                ),
                "save_txt": (
                    "BOOLEAN",
                    {
                        "default": True,
                        "tooltip": "Create a human-readable text summary.",
                    },
                ),
                "save_db": (
                    "BOOLEAN",
                    {
                        "default": False,
                        "tooltip": "Send metadata to the configured database handler, if any.",
                    },
                ),
                "save_individual_discovery": (
                    ...
                    {
                        "default": False,
                        "tooltip": "Persist per-run workflow discovery reports in addition to the central log.",
                    },
                ),
                "debug_logging": (
                    "BOOLEAN",
                    {
                        "default": False,
                        "tooltip": "Verbose console logging for troubleshooting.",
                    },
                ),
                "embed_workflow": (
                    "BOOLEAN",
...
# Node registration
NODE_CLASS_MAPPINGS = {
    "MetadataAwareSaveImage_Simple": MetadataAwareSaveImageSimple,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "MetadataAwareSaveImage_Simple": "Save Image with Metadata (Simple)",
}