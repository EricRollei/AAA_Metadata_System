"""
Multi-LoRA Loader for Wan Video (i2v)
Platform-specific loader for Wan Video Image-to-Video LoRAs
Filters to show only LoRAs in L:\stable-diffusion-webui\models\Lora\Wan\i2v
...
class MultiLoRALoaderWanI2V(MultiLoRALoaderBase):
    """
    Multi-LoRA loader specifically for Wan Video (Image-to-Video) LoRAs.
    Only shows LoRAs from the Wan\i2v directory.
    Model-only (no CLIP).
    """
    
    # Platform-specific configuration
    PLATFORM_NAME = "Wan-i2v"
    PLATFORM_DIRECTORY_FILTER = os.path.join("Wan", "i2v")  # Will scan Lora\Wan\i2v
    ...
    def load_multi_loras(self, model, prompt: str,
        ...
        """Load multiple LoRAs for Wan i2v (model only)"""
        ...
        # Create filter info
        active_filters = []
        active_filters.append(f"Platform: Wan i2v")
        ...
        filter_info = f"FILTERS: {' | '.join(active_filters)} | Found {len(filtered_lora_paths)} of {len(self.lora_paths)} Wan i2v LoRAs"
        ...
        for i, (enabled, name, strength) in enumerate(lora_configs, 1):
            ...
            if not lora_path:
                print(f"[Wan-i2v] Warning: LoRA '{name}' not found, skipping slot {i}")
                continue
            ...
            try:
                ...
                print(f"[Wan-i2v] Loaded LoRA {i}: {name} (strength: {strength})")
                
            except Exception as e:
                print(f"[Wan-i2v] Error loading LoRA '{name}' in slot {i}: {str(e)}")
                continue
...
# Node registration
NODE_CLASS_MAPPINGS = {
    "Multi-LoRA Loader Wan-i2v": MultiLoRALoaderWanI2V,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "Multi-LoRA Loader Wan-i2v": "Multi-LoRA Loader (Wan i2v)",
}
